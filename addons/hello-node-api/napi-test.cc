...
static napi_value SayHello(napi_env env, napi_callback_info info) {
  napi_status status;
  napi_value world;
  status = napi_create_string_utf8(env, "Hello world node api", 20, &world);
...
napi_value Init(napi_env env, napi_value exports) {
  ...
  napi_property_descriptor desc[] = {
    DECLARE_NAPI_METHOD("sayHello", SayHello),
    DECLARE_NAPI_METHOD("add", Add)
  };
  status = napi_define_properties(env, exports, sizeof(desc)/sizeof(desc[0]), &desc[0]);
  assert(status == napi_ok);
  return exports;
}
